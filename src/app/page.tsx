...
export default function Home() {
  ...
  // Save every config edit to localStorage. This intentionally includes standalone
  // profile fields such as DOB even before Sheets/search setup exists.
  useEffect(() => {
    try {
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-48">
                        ...
                        <input
                          ...
                          min={1}
                          max={50}
                          value={Math.min(50, Math.max(1, config.limitPerDay || 50))}
                          onChange={(e) => {
                            const raw = Number.parseInt(e.target.value, 10);
                            const next = Number.isFinite(raw) ? Math.min(50, Math.max(1, raw)) : 1;
                            setConfig({ ...config, limitPerDay: next });
                          }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
                    <div>
                      ...
                      <input
                        ...
                        min={1}
                        max={50}
                        value={Math.min(50, Math.max(1, config.limitGlints || 20))}
                        onChange={(e) => { const raw = Number.parseInt(e.target.value, 10); setConfig({ ...config, limitGlints: Number.isFinite(raw) ? Math.min(50, Math.max(1, raw)) : 1 }); }}
                        className="w-full bg-slate-950 border border-slate-700 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-blue-500 font-bold"
                      />
                      <p className="text-xs text-slate-500 mt-1">Maksimal 50 lowongan di Glints (batas sistem).</p>
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        min={1}
                        max={50}
                        value={Math.min(50, Math.max(1, config.limitJobstreet || 20))}
                        onChange={(e) => { const raw = Number.parseInt(e.target.value, 10); setConfig({ ...config, limitJobstreet: Number.isFinite(raw) ? Math.min(50, Math.max(1, raw)) : 1 }); }}
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-1">Maksimal 50 lowongan di Jobstreet (batas sistem).</p>
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        min={1}
                        max={50}
                        value={Math.min(50, Math.max(1, config.limitLinkedin || 20))}
                        onChange={(e) => { const raw = Number.parseInt(e.target.value, 10); setConfig({ ...config, limitLinkedin: Number.isFinite(raw) ? Math.min(50, Math.max(1, raw)) : 1 }); }}
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-1">Maksimal 50 lowongan di LinkedIn (batas sistem).</p>
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        min={1}
                        max={50}
                        value={Math.min(50, Math.max(1, config.limitIndeed || 20))}
                        onChange={(e) => { const raw = Number.parseInt(e.target.value, 10); setConfig({ ...config, limitIndeed: Number.isFinite(raw) ? Math.min(50, Math.max(1, raw)) : 1 }); }}
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-1">Maksimal 50 lowongan di Indeed (batas sistem).</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <select
                      value={config.dateOfBirthDay || ''}
                      onChange={(e) => setConfig({ ...config, dateOfBirthDay: e.target.value })}
                      className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500"
                    >
                    ...
                    <select
                      value={config.dateOfBirthMonth || ''}
                      onChange={(e) => setConfig({ ...config, dateOfBirthMonth: e.target.value })}
                      className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500"
                    >
                    ...
                    <select
                      value={config.dateOfBirthYear || ''}
                      onChange={(e) => setConfig({ ...config, dateOfBirthYear: e.target.value })}
                      className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500"
                    >
                      <option value="">Tahun</option>
                      {Array.from({ length: 101 }, (_, i) => String(new Date().getFullYear() - i)).map(year => (
                        <option key={year} value={year}>{year}</option>
                      ))}
                    </select>
                  </div>
                  <div className="mt-2 p-2.5 bg-slate-950/70 border border-slate-800 rounded text-[11px] text-slate-400">
                    <strong className="text-slate-300">Tersimpan sebagai tiga field editable:</strong> Tanggal = {config.dateOfBirthDay || '—'}, Bulan = {config.dateOfBirthMonth ? ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'][Number(config.dateOfBirthMonth) - 1] : '—'}, Tahun = {config.dateOfBirthYear || '—'}.
                    <span className="block mt-1">Jika ketiganya lengkap, sistem membentuk nilai internal YYYY-MM-DD untuk mesin bot. Gemini tidak boleh menebak DOB.</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500"
                  />
                  <p className="text-[11px] text-slate-500 mt-1">Dipakai untuk checklist bahasa di JobStreet. Jangan isi bahasa yang tidak Anda kuasai.</p>
                </div>
